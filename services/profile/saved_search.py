...
class SavedSearchService:
    ...
    def get_saved_searches(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # filters-Feld von JSON-String in Dict umwandeln (je nach Supabase-Client)
            for it in items:
                flt = it.get("filters")
                if isinstance(flt, str):
                    try:
                        it["filters"] = json.loads(flt)
                    except Exception:
                        # Ungültiges JSON – als leeres Dict behandeln
                        it["filters"] = {}
                elif flt is None:
                    it["filters"] = {}
    ...
    def save_search(
        ...
        length_valid, length_error = validate_length(name, max_length=MAX_SEARCH_NAME_LENGTH)
        if not length_valid:
            return False, length_error or f"Max. {MAX_SEARCH_NAME_LENGTH} Zeichen."
        ...
        try:
            ...
            if count_res.count and count_res.count >= MAX_SAVED_SEARCHES:
                return False, f"Maximal {MAX_SAVED_SEARCHES} Suchaufträge erlaubt."

            # Name bereinigen
            sanitized_name = sanitize_string(name, max_length=MAX_SEARCH_NAME_LENGTH)

            # Filter-JSON vorbereiten (wird in der Spalte 'filters' gespeichert)
    ...
    def get_search_by_id(self, search_id: int) -> Optional[Dict[str, Any]]:
        ...
        Returns:
            Suchauftrag-Dictionary oder None
        """
        user_id = self._get_current_user_id()
        if not user_id:
            return None
